...
void Chunk_Setup() {
    ...
    //--------------------------------------------------------------------------
    /** - get the number of chunks to build for i2o and o2i */
    //--------------------------------------------------------------------------
    // one chunk per rank in each dimension
    auto get_nchunks = [](const int shift[3], const Topology* topo_in, const Topology* topo_out) {
        // get the real start and end index that will exist in both topologies
        int start[3], end[3];
        topo_in->cmpt_intersect_id(shift, topo_out, start, end);

        int n_chunk = 1;
        for (int id = 0; id < 3; ++id) {
            // find the rank that will own the starting/ending point of the input topology
            const int start_rank = topo_out->cmpt_rank_fromid(start[id] + shift[id]);
            const int end_rank   = topo_out->cmpt_rank_fromid(end[id] + shift[id]);

            // accumulate the number of chunks over the dimensions
            n_chunk *= (end_rank - start_rank);
        }
        // as we have one chunk per rank, we can now compute the number of chunks
        return n_chunk;
    };

    i2o_nchunks_ = get_nchunks(shift, topo_in_, topo_out_);
    o2i_nchunks_ = get_nchunks(shift, topo_out_, topo_in_);

    // allocate the arrays
    i2o_chunks_ = reinterpret_cast<MemChunk*>(m_calloc(i2o_nchunks_ * sizeof(MemChunk)));
    o2i_chunks_ = reinterpret_cast<MemChunk*>(m_calloc(o2i_nchunks_ * sizeof(MemChunk)));
...
void SwitchTopoX::SubCom_SplitComm(){
    ...
    for (int ib = 0; ib < in_chunks_; ib++) {
        mycolor                     = m_min(mycolor, ichunks_[ib]->dest_rank);
        inMyGroup[i2o_destRank_[ib]] = true;
    }
    for (int ib = 0; ib < ochunks_; ib++) {
        mycolor                      = m_min(mycolor, ochunks_[ib]->dest_rank);
...
void SwitchTopoX::SubCom_UpdateRanks(const int n_chunks, MemChunk* chunks) {
    BEGIN_FUNC;
    //--------------------------------------------------------------------------
    ...
    for (int ic = 0; ic < n_chunks; ++ic) {
        ...
        const int in_dest_rank = chunks[ic].dest_rank;
        // replace by the one in the subcommunicator
        chunks[ic].dest_rank = subRanks[in_dest_rank];
    }
