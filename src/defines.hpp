...
#ifndef NDEBUG
#define FLUPS_CHECK0(a, b) \
    if (!(a)) {         \
        FLUPS_ERROR(b);    \
    }
#define FLUPS_CHECK1(a, b, c)      \
    if (!(a)) {                 \
        char msg_chk[1024];     \
        sprintf(msg_chk, b, c); \
        FLUPS_ERROR(msg_chk);      \
    }
#define FLUPS_CHECK2(a, b, c, d)      \
    if (!(a)) {                    \
        char msg_chk[1024];        \
        sprintf(msg_chk, b, c, d); \
        FLUPS_ERROR(msg_chk);         \
    }
#define FLUPS_CHECK3(a, b, c, d, e)      \
    if (!(a)) {                       \
        char msg_chk[1024];           \
        sprintf(msg_chk, b, c, d, e); \
        FLUPS_ERROR(msg_chk);            \
    }
#define FLUPS_CHECK4(a, b, c, d, e, f)      \
    if (!(a)) {                          \
        char msg_chk[1024];              \
...
#else
#define FLUPS_CHECK0(a, b) ((void)0);
#define FLUPS_CHECK1(a, b, c) ((void)0);
#define FLUPS_CHECK2(a, b, c, d) ((void)0);
#define FLUPS_CHECK3(a, b, c, d, e) ((void)0);
#define FLUPS_CHECK4(a, b, c, d, e, f) ((void)0);
#endif
...
#define FLUPS_ISALIGNED(a) \
    { ((uintptr_t)(const void*)a) % FLUPS_ALIGNMENT == 0; }
