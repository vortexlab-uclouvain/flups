...
def check_res(i, file):
    ...
    for line in csv.reader(fcurr,delimiter=' '):
        ...
        if vals[0] < 1e-14: #absolute error because ref is 0
            ...
            err2   = err2   + abs((vals[0]-float(buff[3])))
            errInf = errInf + abs((vals[1]-float(buff[4])))
            err2   = err2   + abs((vals[0]-float(buff[5])))
            errInf = errInf + abs((vals[1]-float(buff[6])))
        else:
            err2   = abs((vals[0]-float(buff[1]))/vals[0])
            errInf = abs((vals[1]-float(buff[2]))/vals[1])
            err2   = err2   + abs((vals[0]-float(buff[3]))/vals[0])
            errInf = errInf + abs((vals[1]-float(buff[4]))/vals[1])
            err2   = err2   + abs((vals[0]-float(buff[5]))/vals[0])
            errInf = errInf + abs((vals[1]-float(buff[6]))/vals[1])
