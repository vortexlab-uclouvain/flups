...
def main():
    ...
    # Prepare data arrays
    local_size = nmem[0] * nmem[1] * nmem[2] * 3  # 3 components
    ...
    for i2 in range(nloc[2]):
        for i1 in range(nloc[1]):
            for i0 in range(nloc[0]):
                for comp in range(3):
                    idx = pf.locID(0, i0, i1, i2, comp, topo.axis, nmem, 1)
    ...
    for i2 in range(nloc[2]):
        for i1 in range(nloc[1]):
            for i0 in range(nloc[0]):
                vx_idx = pf.locID(0, i0, i1, i2, 0, topo.axis, nmem, 1)
                vy_idx = pf.locID(0, i0, i1, i2, 1, topo.axis, nmem, 1)
                vz_idx = pf.locID(0, i0, i1, i2, 2, topo.axis, nmem, 1)
