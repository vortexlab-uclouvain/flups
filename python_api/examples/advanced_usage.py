...
def main():
    ...
    # Display profiler - all ranks must call this (it's a collective MPI operation)
    if rank == 0:
        print("\nProfiling results:")
    prof.display()  # ALL ranks must call this
    ...
    try:
        ...
        if rank == 0:
            print(f"Physical topology axis: {topo_phys.axis}")

        # Manual operations (simplified - actual implementation needs buffer)
        solver2.solve(result, test_data)
